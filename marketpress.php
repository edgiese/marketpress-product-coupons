...
';
  ...
  function shipping_price($format = false, $cart = false) {
    ...
    if ( $coupon = $this->coupon_value($coupon_code, $cart, $total) )
  ...
  function update_cart() {
    ...
    if (isset($_POST['empty_cart'])) { //empty cart contents
      ...
      //add coupon code
            if (is_multisite())
              $cart = $global_cart[$current_blog_id];
      if (!empty($_POST['coupon_code'])) {
        if ('' == ($coupon_error=$this->check_coupon($_POST['coupon_code'],$cart))) {
          //get coupon code
          if (is_multisite()) {
            global $blog_id;
          ...
          $this->cart_checkout_error( __($coupon_error, 'mp') );
        }
      }
      ...
      if ($this->checkout_error == false) {
        ...
        //coupon line
        if ( $coupon = $this->coupon_value($this->get_coupon_code(), $cart, $total) )
          $total = $coupon['new_total'];
  ...
  // returns array('error'=>msg, 'code'=>code, 'data'=>coupon_data)
  function get_coupon_data($code) {
    $coupon_code = preg_replace('/[^A-Z0-9_-]/', '', strtoupper($code));

    //empty code
    if (!$coupon_code)
      return array('error'=>'Illegal Coupon Code');

    $coupons = get_option('mp_coupons');

    //no record for code
    if (!isset($coupons[$coupon_code]) || !is_array($coupons[$coupon_code]))
      return array('error'=>"Coupon Code $coupon_code does not exist");

    return array('code'=>$coupon_code,'data'=>$coupons[$coupon_code], 'alldata'=>$coupons);
  }

  function sku_rule_match($rule, $sku) {
    // allow a few common special chars to be in skus without breaking test
    $rule=str_replace('.','\\.',$rule);
    $rule=str_replace('-','\\-',$rule);

    // substitute simple wildcards with preg wildcards
    $rule=str_replace('?','.?',$rule);
    $rule=str_replace('*','.*',$rule);

    $retval = preg_match('/'.$rule.'/',$sku);
    // echo "<p>rule=$rule  sku=$sku  retval=".($retval ? 'True' : 'false') . "</p>";
    return $retval;
  }

  function coupon_products($coupon_data, $cart) {
    $pieces = explode(';',$coupon_data['products']);
    $required_sku = explode(',',$pieces[0]);
    $ored_sku = isset($pieces[1]) ? explode(',', $pieces[1]) : array();
    $prices = isset($pieces[2]) ? explode(',', $pieces[2]) : array($coupon_data['discount']);
    $found_sku=array();
    for ($i = 0; $i < sizeof($required_sku); ++$i)
      $found_sku[$i] = False;
    $ored_found=0;
    ...
    foreach ($cart as $product_id => $variations) {
      ...
      if (!is_array($skus))
        $skus=array($skus);
      foreach ($skus as $sku) {
        foreach ($required_sku as $i => $rsku) {
          if ($this->sku_rule_match($rsku, $sku)) {
            $found_sku[$i] = True;
          }
        }
        foreach ($ored_sku as $i => $osku) {
          if ($this->sku_rule_match($osku, $sku))
            ++$ored_found;
        }
      }
    }
    for ($i = 0; $i < sizeof($required_sku); ++$i) {
      if (!$found_sku[$i])
        return array('error'=>$coupon_data['message']);
    }
    if ($ored_found == 0 && sizeof($ored_sku) > 0) {
      return array('error'=>$coupon_data['message']);
    }
    if ($ored_found > sizeof($prices)-1)
      $ored_found = sizeof($prices)-1;

    return array('discount'=>$prices[$ored_found]);
  }

  //checks a coupon code for validity. Returns error string or empty string if no error
  function check_coupon($code, $cart = null) {
    $data=$this->get_coupon_data($code);
    if (isset($data['error']))
      return $data['error'];
    ...
    $coupon_code=$data['code'];
		...
		$return = apply_filters('mp_coupon_check', null, $coupon_code, $data['alldata']);
		if ( !is_null($return) )
			return $return;

    //start date not valid yet
    if (time() < $coupon_data['start'])
      return "Coupon Code $coupon_code Code is not active until ".date('m/d/Y',$coupon_data['start']);

    //if end date and expired
    if ($coupon_data['end'] && time() > $coupon_data['end'])
      return "Coupon Code $coupon_code has expired.";

    //check remaining uses
    if ($coupon_data['uses'] && (intval($coupon_data['uses']) - intval(@$coupon_data['used'])) <= 0)
      return "Coupon Code $coupon_code has been used up.";

    if ($cart != null) {
      // check coupon against products for validity
      $data=$this->coupon_products($coupon_data,$cart);
      if (isset($data['error']))
        return $data['error'];
    }
  ...
  function coupon_value($code, $cart, $total) {
    $data=$this->get_coupon_data($code);
    if (isset($data['error']))
      return false;
    if ('' != $this->check_coupon($code, $cart))
      return false;
    $coupon_data=$data['data'];
    $coupon_code=$data['code'];
    $product_coupon_data=$this->coupon_products($coupon_data,$cart);
    if (isset($product_coupon_data['error']))
      return false;
    // product-wise discount overrides overall discount
    $coupon_data['discount']=$product_coupon_data['discount'];

    if ($coupon_data['discount_type'] == 'amt') {
      $new_total = round($total - $coupon_data['discount'], 2);
      $new_total = ($new_total < 0) ? 0.00 : $new_total;
      $discount = '-' . $this->format_currency('', $coupon_data['discount']);
      $return = array('discount' => $discount, 'new_total' => $new_total);
    } else {
      $new_total = round($total - ($total * ($coupon_data['discount'] * 0.01)), 2);
      $new_total = ($new_total < 0) ? 0.00 : $new_total;
  ...
  function create_order($order_id, $cart, $shipping_info, $payment_info, $paid, $user_id = false, $shipping_total = false, $tax_total = false, $coupon_code = false) {
    ...
    if ( $coupon = $this->coupon_value($code, $cart, 9999999999) ) {
    ...
    ?>
  	...
  	switch( $tab ) {
        ...
        if (isset($_POST['submit_settings'])) {
          ...
          $coupons[$new_coupon_code]['products'] = $_POST['products'];
          $coupons[$new_coupon_code]['message'] = $_POST['message'];
          ...
          <div class="inside">
            ...
            if ( isset( $error ) && $error ) {
          	...
          	//setup defaults
						...
						$products = isset($coupons[$new_coupon_code]['products']) ? $coupons[$new_coupon_code]['products'] : '';
						$message = isset($coupons[$new_coupon_code]['message']) ? $coupons[$new_coupon_code]['message'] : '';
            ...
            <tr>
              ...
              <th><?php _e('Products', 'mp') ?></th>
              <th><?php _e('Failure Message', 'mp') ?></th>
            ...
            <tr>
              ...
              <td>
                <input value="<?php echo $products; ?>" size="31" name="products" type="text" />
              </td>
              <td>
                <input value="<?php echo $message; ?>" size="31" name="message" type="text" />
              </td>
              <td>
